...
export async function GET(req: NextRequest) {
  ...
  if (!issuer) {
    return NextResponse.json({ error: "NEXT_PUBLIC_AUTH_ISSUER missing" }, { status: 500 });
  }

  let nextPath = req.nextUrl.searchParams.get("next") ?? "/battle";
  ...
  res.cookies.set(COOKIE_VER, verifier, base);
  res.cookies.set(COOKIE_ST, state, base);
  res.cookies.set(COOKIE_NEXT, nextPath, base);
  return res;
}
